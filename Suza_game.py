...
def play_process(message:Message , bot):
    ...
    if (message.text == "+"
            or message.text == "+++"
            or message.text == "спасибо"
            or message.text == "спс"
            or message.text == "сяб"
            or message.text == "пасибо"
            or message.text == "пасяб"
            or message.text == "спс"
            or message.text == "благодарю"):
        robber, victim, phrase = thanks_giving(robber,victim, phrase)
    elif message.text == "-":
        robber, victim, phrase = negative_giving(robber, victim, phrase)
    elif (message.text == "украсть"
          or message.text == "скрасть"
          or message.text == "спиздить"
          or message.text == "обокрасть"
          or message.text == "обчистить"
          or message.text == "своровать"):
        robber, victim, phrase = attempted_theft(robber, victim, phrase)
    elif (message.text == "пнуть"
          or message.text == "опустить"
          or message.text == "отпиздить"
          or message.text == "унизить"
          or message.text == "зачмырить"):
        robber, victim, phrase = kicking(robber, victim, phrase)
    elif (message.text == "обнять"
          or message.text == "обнимашки"
          or message.text == "затискать"
          or message.text == "потискать"):
        robber, victim, phrase, photo = hugging(robber, victim, phrase, photo)
    elif (message.text == "поцеловать"
          or message.text == "поцелуй"
          or message.text == "чмок"
          or message.text == "целовашки"):
...
def negative_giving(robber, victim, phrase):
    if robber[0] == victim[0]:
        phrase = ("Смотрите-ка сам себе пинка отвесить умудрился! "
                  "Мазохист какой-то")
    elif victim[0] == 6026030050:
...
def attempted_theft(robber, victim, phrase): # проверить на сам себя
    ...
    if trying > 5:
        if victim[4] > modificator:
            ...
            phrase = (f"{robber[3]} пытался обокрасть {victim[3]}. "
                      f"Да только красть у него нечего."
                      f"\n{robber[3]}  {robber[4]} : "
                      f"\u2699\n{victim[3]}  {victim[4]} : \u2699")
    else:
...
def kicking(robber, victim, phrase):
    ...
    modificator = random.randint(1, 15)
    if robber[0] == victim[0]:
        phrase = ("Ну, как бы если тебе хочется пинать самого себя - "
                  "никто не против. Забавное надо сказать зрелище.")
    elif victim[0] == 6026030050:
        robber[5] -= 50
        robber[6] = set_status(robber[5])
        phrase = (f"Ты на кого клешню пытаешься поднять, "
                  f"мешок кожанный?!\n{robber[3]}  \U0001F3C5{robber[5]} "
                  f"\n{robber[3]} \U0001F3CD : "
                  f"{robber[6]} \U0001F3C5 : {robber[5]}")
    else:
        if trying > 8:
            ...
            phrase = (f"{robber[3]} пнул {victim[3]}.\n{robber[3]} "
                      f"+{modificator}\U0001F3C5 / {victim[3]} "
                      f"-{modificator}\U0001F3C5. \n{robber[3]}  \U0001F3CD : "
                      f"{robber[6]} \U0001F3C5 : {robber[5]}\n{victim[3]}  "
                      f"\U0001F3CD : {victim[6]} \U0001F3C5 : {victim[5]}")
        elif 3 < trying <= 8:
            phrase = (f"{robber[3]} пытался пнуть {victim[3]}. "
                      f"Но из этого ничего не вышло."
                      f"\n{robber[3]}  \U0001F3CD : {robber[6]} "
                      f"\U0001F3C5 : {robber[5]}\n{victim[3]} "
                      f"\U0001F3CD : {victim[6]} \U0001F3C5 : {victim[5]}")
        else:
            ...
            victim[6] = set_status(victim[5])
            phrase = (f"{robber[3]} пытался пнуть {victim[3]}. "
                      f"Но поскользнулся и только задницу себе отбил"
                      f"\n {robber[3]} \U0001F3CD{robber[6]} "
                      f"\U0001F3C5{robber[5]} / {victim[3]} "
                      f"\U0001F3CD{victim[6]} {victim[5]}")
    return robber, victim, phrase

def kissing(robber, victim, phrase, photo):
    if victim[0] == 6026030050:
        photo = open("hug.jpg", 'rb')
        phrase = (f"{robber[3]} поцеловал {victim[3]}. "
                  f"Ммммм, целовашчки!\n{robber[3]}  \U0001F3CD : "
                  f"{robber[6]} \U0001F3C5 : {robber[5]}\n{victim[3]}  "
                  f"\U0001F3CD : {victim[6]} \U0001F3C5 : {victim[5]}")
        ...
        victim[6] = set_status(victim[5])
        phrase = (f"{robber[3]} поцеловал {victim[3]}. "
                  f"Целовашечки это здорово.\n{victim[3]}  \U0001F3C5 : "
                  f"+7\n{robber[3]}  \U0001F3CD : {robber[6]} \U0001F3C5 : "
                  f"{robber[5]}\n{victim[3]} \U0001F3CD : "
                  f"{victim[6]} \U0001F3C5 : {victim[5]}")
    return robber, victim, phrase, photo

def hugging(robber, victim, phrase, photo):
    if victim[0] == 6026030050:
        photo = open("hug.jpg", 'rb')
        phrase = (f"{robber[3]} обнял {victim[3]}. "
                  f"Обнимашки это так мило.\n{robber[3]}  \U0001F3CD : "
                  f"{robber[6]} \U0001F3C5 : {robber[5]}\n{victim[3]}  "
                  f"\U0001F3CD : {victim[6]} \U0001F3C5 : {victim[5]}")
    elif victim[0] == robber[0]:
        robber[5] += 3
        ...
        phrase = (f"{robber[3]} обнял {victim[3]}. "
                  f"Обнимашки это так мило.\n{robber[3]} "
                  f"\U0001F3CD{robber[6]} \U0001F3C5{robber[5]} / "
                  f"{victim[3]} \U0001F3CD{victim[6]} \U0001F3C5{victim[5]}")
    return robber, victim, phrase, photo
...
def gifting(robber, victim, phrase):
    if victim[0] == 6026030050:
        ...
        robber[5] += 3
        robber[6] = set_status(robber[5])
        phrase = (f"{robber[3]} подарил Сузе 7 \u2699.Спасибо за подарочек). "
                  f"Очень приятно.\nАвторитет {robber[3]} повышен, "
                  f"а {victim[3]} получил немножко запчастей для мототайки."
                  f"\n{robber[3]} \U0001F3C5 : "
                  f"{robber[5]}\n{victim[3]} \u2699 : {victim[4]}")
...
def catting(robber, victim, phrase, text):
    cat_names = [
        "кот", "котик", "котейка", "котэ", "котя", "котофей", "котяра",
        "cat", "kot", "koteika", "kitty", "cote"]
    if text == "погладить":
        action_text = "погладил котика"
    else:
        action_text = "почесал котику спинку"
    if (str(victim[2]).lower() in cat_names
            or str(victim[3]).lower() in cat_names):
        if robber[0] == victim[0]:
            victim[5] -= 2
            victim[6] = set_status(victim[5])
            phrase = (f"Кот хватит вылизывать яйца при всех! "
                      f"\n{victim[3]} \U0001F3CD{victim[6]} "
                      f"\U0001F3C5{victim[5]}")
        else:
            victim[5] += 5
            victim[6] = set_status(victim[5])
            robber[5] += 5
            robber[6] = set_status(robber[5])
            phrase = (f"{robber[3]} {action_text} ({victim[3]}). "
                      f"Оба получили удовольствие ).\n{robber[3]} "
                      f"\U0001F3CD{robber[6]} \U0001F3C5{robber[5]} / "
                      f"{victim[3]} \U0001F3CD{victim[6]} "
                      f"\U0001F3C5{victim[5]}")
    return robber, victim, phrase
...
def asking_status(robber, phrase, message):
    with sqlite3.connect("game.db") as con:
        ...
        phrase = (f"{robber[0]}/{robber[1]}/{robber[2]} {robber[3]}\n"
                  ...
                  f"Авторитет: \U0001F3C5 : {robber[5]}  \U00002699 : "
                  f"{robber[6]}\n{robber[7]}")
...
def saving_res(robber, victim, phrase, message):
    with sqlite3.connect("game.db") as con:
        ...
        cursor.execute(f"UPDATE '{message.chat.id}' "
                       ...
                       f"status = '{robber[6]}', married = '{robber[7]}' "
                       f"WHERE user_id = {robber[0]}")
        con.commit()
        if robber[0] != victim[0]:
            cursor.execute(f"UPDATE '{message.chat.id}' "
                           ...
                           f"credibility = {victim[5]}, "
                           f"status = '{victim[6]}', married = '{victim[7]}' "
                           f"WHERE user_id = {victim[0]}")
            con.commit()
